...
//    boost::log::add_file_log("dbmgr.log", keywords::auto_flush = true);  
    auto file_sink = logging::add_file_log  
        (  
        keywords::file_name = "client_%N.log",                                        /*< file name pattern >*/  
...
int readcfg(int & port,std::string& host)
{
    boost::property_tree::ptree m_pt, tag_setting;
    read_ini("clientconf.ini", m_pt);
    tag_setting = m_pt.get_child("config");
    port   = tag_setting.get<int>("port", 20000);   // tcp listen port
    host   = tag_setting.get<std::string>("ip", "127.0.0.1");   // tcp listen port
