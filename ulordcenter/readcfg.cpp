...
//    boost::log::add_file_log("dbmgr.log", keywords::auto_flush = true);  
    auto file_sink = logging::add_file_log  
        (  
        keywords::file_name = "ucenter_%N.log",                                        /*< file name pattern >*/  
...
int readcfg(int & port,int &thread,  string & dbuser, string &dbhost,string &dbpass)
{
    boost::property_tree::ptree m_pt, tag_setting;
    read_ini("ucenter.ini", m_pt);
    tag_setting = m_pt.get_child("config");
    port     = tag_setting.get<int>("port", 1070);   // tcp listen port
    thread   = tag_setting.get<int>("thread", 1070);   // tcp listen port
    ...
    g_privkeystr= tag_setting.get<string>("privkey", "");   // tcp listen port
    g_pubkeystr= tag_setting.get<string>("pubkey", "");   // tcp listen port
