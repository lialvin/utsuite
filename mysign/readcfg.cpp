...
//    boost::log::add_file_log("dbmgr.log", keywords::auto_flush = true);  
    auto file_sink = logging::add_file_log  
        (  
        keywords::file_name = "mysign_%N.log",                                        /*< file name pattern >*/  
...
int readcfg(int & startheight, int  &endheight, string & dbuser, string &dbhost,string &dbpass,string& privkey,string& pubkey)
{
    boost::property_tree::ptree m_pt, tag_setting;
    read_ini("mysign.ini", m_pt);
    ...
    privkey  = tag_setting.get<string>("privkey", "");   // tcp listen port
    pubkey   = tag_setting.get<string>("pubkey", "");   // tcp listen port
